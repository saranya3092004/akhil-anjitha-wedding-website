...
const tileSpans = ['row-span-2', '', '', 'row-span-2', ''];
...
export default function Gallery() {
  ...
  const closeLightbox = () => setActiveImageIdx(null);
  ...
  // Lock body scroll while the lightbox is open (portal means the page behind can still scroll otherwise)
  useEffect(() => {
    if (activeImageIdx === null) return;
    ...
    <AnimatePresence>
      {activeImageIdx !== null && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 bg-maroon-950/98 backdrop-blur-md z-[9999] flex items-center justify-center p-4 md:p-10"
          onClick={closeLightbox}
        >
          {/* Close button — now rendered directly under <body>, immune to any parent's overflow-hidden */}
          <button
            aria-label="Close gallery"
            className="fixed flex items-center justify-center rounded-full transition-all z-[10000] cursor-pointer shadow-xl bg-maroon-800 hover:bg-maroon-900 text-white border-2 border-gold-300/60"
            style={{
              width: '48px',
              height: '48px',
          ...
          {/* Persistent bottom "Back to Gallery" bar, mobile only */}
          <button
            onClick={(e) => {
              e.stopPropagation();
              closeLightbox();
            }}
            className="fixed bottom-0 left-0 right-0 md:hidden z-[10000] flex items-center justify-center gap-2 py-4 bg-maroon-900/95 backdrop-blur-md border-t border-gold-400/25 text-gold-100 text-sm font-semibold font-cormorant tracking-wide"
          ...
          >
            ...
            <p className="mt-6 text-center text-[10px] uppercase tracking-[0.2em] text-gold-400/60 font-cormorant font-bold hidden md:block">
              Swipe or scroll sideways to view next photo
            </p>
          </motion.div>
        </motion.div>
      )}
  ...
  return (
    <section id="gallery" className="py-24 bg-maroon-950/40 relative overflow-hidden border-b border-gold-400/10">
      ...
      {/* Portal the lightbox to <body>, escaping this section's overflow-hidden entirely */}
